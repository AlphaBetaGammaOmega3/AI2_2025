...
const AdminHomePage = () => {
  ...
  const handleDelete = async (iduser) => {
    ...
    try {
      const res = await fetch(`/api/users/${iduser}`, { method: 'DELETE' });
      if (res.ok) {
    ...
    <div className="admin-container">
      ...
      <div className="admin-content">
        <div className="user-list">
          ...
          {users.map(user => (
            <div key={user.iduser} className="user-item" onClick={() => setSelected(user)}>
              ...
              <button className="edit" onClick={() => navigate(`/admin/utilizadores/editar/${user.iduser}`)}>Editar</button>
              <button className="remove" onClick={(e) => {
                e.stopPropagation(); // Evita selecionar ao clicar em remover
          ...
          {selected ? (
            <>
              ...
              <div className="user-detail-body">
                ...
                <p><strong>Tipo:</strong> {selected.tipouser?.nome}</p>
              </div>
            </>
          ) : (
